...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['novo_nome_usuario'])) {
        ...
        // Prepara a declaração SQL para atualizar o nome de usuário do usuário
        $stmt = $conexao->prepare("UPDATE usuarios SET nome_usuario = ? WHERE nome_usuario = ?");

        // Verifica se a preparação da declaração teve sucesso
        if ($stmt) {
            // Associa os parâmetros à declaração
            $stmt->bind_param("ss", $_POST['novo_nome_usuario'], $_POST['nome_usuario']);

            // Executa a declaração
            if ($stmt->execute()) {
                echo "Nome de usuário atualizado com sucesso.";
            } else {
                echo "Erro ao atualizar o nome de usuário: " . $stmt->error;
            }
